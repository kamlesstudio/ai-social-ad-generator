...
def process_video_generation_db(task_id: str, request_data: Dict[str, Any], user_id: str):
    ...
    try:
        ...
        try:
            ...
            db_service.update_video_status(user_id, task_id, "failed", error_message=f"Veo service error: {str(e)}")
            return
        ...
        if duration not in [4, 6, 8]:
            duration = 6
            logger.warning(f"⚠️ Invalid duration, defaulting to 6 seconds")
        ...
        logger.info(f"🎬 Parameters: duration={duration}s, aspect={aspect_ratio}, audio={generate_audio}")
        ...
        if generation_mode in ["image", "first-last"]:
            if first_frame_path:
                ...
                if first_frame_image:
                    ...
                    logger.warning(f"⚠️ First frame not available, falling back to text-to-video")
                    generation_mode = "text"
            else:
                logger.warning(f"⚠️ No first frame path provided, falling back to text-to-video")
                generation_mode = "text"
        ...
        if generation_mode == "first-last":
            if last_frame_path:
                ...
                if last_frame_image:
                    ...
                    logger.warning(f"⚠️ Last frame not available, falling back to image-to-video")
                    generation_mode = "image"
            else:
                logger.warning(f"⚠️ No last frame path provided, falling back to image-to-video")
                generation_mode = "image"
        ...
        try:
            if generation_mode == "image" and first_frame_image:
                ...
                prompt = f"Product showcase of {product_name}. {product_description}. Modern style, {aspect_ratio} format. Professional, clean, visually appealing."
                ...
                prompt = f"Transition showcasing {product_name}. {product_description}. Smooth, professional animation. Clean product presentation."
                logger.info(f"📝 Prompt: {prompt[:100]}...")
                ...
                video_url = veo_service.generate_with_first_last_frame(
                    ...
                    duration=min(duration, 8),  # First/Last supports up to 8 seconds
                    aspect_ratio=aspect_ratio,
                    generate_audio=generate_audio,
                    use_lite=False
                )
            ...
            db_service.update_video_status(user_id, task_id, "failed", error_message=f"Generation error: {str(gen_error)}")
            return
